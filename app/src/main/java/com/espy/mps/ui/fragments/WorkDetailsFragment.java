...
import static com.espy.mps.utils.DialogueUtils.WORK_DEATAILS_UPLOAD_ERROR;
import static com.espy.mps.utils.DialogueUtils.WORK_DEATAILS_UPLOAD_SUCCESS;

public class WorkDetailsFragment extends BaseFragment implements DialogInteractionListener, WorkDetailsCallBack, GPSListener {
    ...
    ArrayList<String> workStatusList = new ArrayList<>();
    ArrayAdapter<String> workStatusAdapter;
    ...
    private void getStatus(){
        ...
        workStatusList.add("Select");
        workStatusList.add("Pending");
        workStatusList.add("Start");
        workStatusList.add("On going");
        workStatusList.add("Completed");
    ...
    private void populatetCustomerStatus(){
        workStatusAdapter = new ArrayAdapter<>(getActivity(), R.layout.spinner_text, workStatusList);
        workStatusAdapter.setDropDownViewResource( android.R.layout.simple_spinner_dropdown_item);
        spnStatus.setAdapter(workStatusAdapter);
    }

    private void submitData(){
        projectid = projectcList.get(spnProject.getSelectedItemPosition()).getProject_id();
        current_status = String.valueOf(spnStatus.getSelectedItemPosition());
...
//                pictureIntent.putExtra("android.intent.extras.CAMERA_FACING", 1);
    ...
    public void onPositiveResponse(int requestCode) {
        if (requestCode == WORK_DEATAILS_UPLOAD_SUCCESS){
    ...
    public void onDetailsUploadSuccess(String response) {
        dismissProgress();
        new DialogueUtils(getActivity()).showSuccessDialog("SUCCESS", "Work details uploaded",
                this, WORK_DEATAILS_UPLOAD_SUCCESS);
    ...
    public void onApiErrorResponse(String message, int type) {
        ...
        }else if (type == WORK_DEATAILS_UPLOAD_ERROR){
            new DialogueUtils(getActivity()).showErrorDialog("OOPS!", "Work details upload failed",
                    this, WORK_DEATAILS_UPLOAD_ERROR);
        }
    ...
    public void onLocationUpdate(Location location, int i) {
        if (location != null) {
            ...
            new WebserviceUtils(getActivity()).insertWorkDetails(this, titleTxt, remarks, imageFilePath, current_status, projectid);
        }else {
            dismissProgress();
