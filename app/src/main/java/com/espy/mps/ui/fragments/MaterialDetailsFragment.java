...
public class MaterialDetailsFragment extends BaseFragment implements DialogInteractionListener, WorkDetailsCallBack, GPSListener {
...
//                pictureIntent.putExtra("android.intent.extras.CAMERA_FACING", 1);
    ...
    public void onLocationUpdate(Location location, int i) {
        if (location != null) {
            ...
            new WebserviceUtils(getActivity()).insertMaterialDetails(this, titleTxt, remarks, imageFilePath, vehicleNoTxt, projectid);
        }else {
            dismissProgress();
    ...
    public void onDetailsUploadSuccess(String response) {
        dismissProgress();
        new DialogueUtils(getActivity()).showSuccessDialog("SUCCESS", "Materials used details uploaded",
